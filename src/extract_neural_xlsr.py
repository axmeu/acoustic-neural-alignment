...
def extract_file(wav_path, group, feature_extractor, model, layer, device):
...
def extract_neural_xlsr(table_path, output_path, model_name, layer, device):
    ...
    for wav_path, group in tqdm(df.groupby("wav_path"), desc="XLSR extraction"):
        try:
            results = extract_file(
                wav_path, group, feature_extractor, model, layer, device
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="outputs/features_xlsr.npz")
